...
class PostgresDataManager:
    """
    A class for inserting and updating data in a PostgreSQL database using SQLAlchemy ORM.
    """
    ...
    def insert_record(self, model: Type[Base], record: Dict[str, any]) -> None:
        ...
        :param record: A dictionary containing data for the single record to insert.
        """
        try:
    ...
    def insert(self, model: Type[Base], data: List[Dict[str, any]]) -> None:
        ...
        :param model: The ORM model class corresponding to the table.
        :param data: A list of dictionaries containing data for multiple records to insert.
        """
        try:
    ...
    def update(self, model: Type[Base], conflict_column: str,
        ...
        :param model: The ORM model class corresponding to the table.
        :param conflict_column: The column to check for conflicts (usually the primary key).
        ...
        try:
            ...
            if record:
                ...
                print(
                    f"Updated record {conflict_column} = {update_data[conflict_column]}")
            else:
                print(
                    f"No record found with {conflict_column} = {update_data[conflict_column]}")
            self.session.commit()
        except SQLAlchemyError as e:
            self.session.rollback()
